/**
 * game.js — Test Incident Room v3.1
...
const DIFF_SETTINGS = { easy:{sec:60,pen:30}, medium:{sec:35,pen:60}, hard:{sec:18,pen:100} };
...
function doRegister() {
  ...
  if (!name||!user||!pass) { err.textContent='Please fill in all fields.'; err.style.display='block'; return; }
  if (pass.length < 4) { err.textContent='Password must be at least 4 characters.'; err.style.display='block'; return; }
  if (!/^[a-z0-9_]+$/.test(user)) { err.textContent='Username: letters, numbers, underscores only.'; err.style.display='block'; return; }
  const users = getUsers();
  if (users[user]) { err.textContent='Username already taken.'; err.style.display='block'; return; }
...
function doLogin() {
  ...
  const users = getUsers();
  if (!users[user]||users[user].password!==pass) { err.textContent='Incorrect username or password.'; err.style.display='block'; return; }
  currentUser = { username: user, displayName: users[user].displayName };
  afterLogin();
}

function afterLogin() {
  document.getElementById('intro-username').textContent = currentUser.displayName + '  (@' + currentUser.username + ')';
  showScreen('intro');
}

function doLogout() { currentUser = null; showScreen('auth'); }
...
function startGame(diff) {
  ...
  score=0; health=100; streak=0; gameTime=480; selectedId=null; actionLock=false;
  resolvedSet.clear(); decisions.length=0;
  clearInterval(clockInterval); clearInterval(countdownInterval);
  showScreen('room'); renderFeed(); updateHUD();
  document.getElementById('logbox').innerHTML =
    `<div class="log-entry"><span class="log-time">08:00</span><span style="color:var(--text-secondary)">Crisis room active — ${diff.toUpperCase()} mode.</span></div>`;
  clockInterval = setInterval(() => {
    gameTime++;
    const h=Math.floor(gameTime/60), m=gameTime%60;
    document.getElementById('h-time').textContent = `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
  }, 1000);
}
...
function renderFeed() {
  ...
  if (remaining.length === 0) {
    feed.innerHTML = `<div style="text-align:center;padding:28px;color:var(--text-secondary);font-size:14px;">All incidents resolved!</div>`;
...
function selectIncident(id) {
  ...
  countdownInterval = setInterval(() => {
    ...
    if (pill) { pill.textContent = incCountdown+'s'; pill.className='countdown'+(incCountdown<=5?' urgent':''); }
    if (incCountdown <= 0) { clearInterval(countdownInterval); timeoutIncident(id); }
  }, 1000);
}
...
function timeoutIncident(id) {
  ...
  streak = 0;
  resolvedSet.add(id);
  decisions.push({ title:inc.title, action:'TIMED OUT', correct:false, points:-penaltyOnTimeout });
  ...
  setTimeout(() => {
    actionLock=false; closeModal(); renderFeed();
...
function resolveAction(incId, shuffledIdx) {
  ...
  if (pts < 0) { health=Math.max(0,health+Math.round(pts/3)); streak=0; } else { streak++; }
  resolvedSet.add(incId);
  decisions.push({ title:inc.title, action:action.l, correct:action.c, points:pts });
...
function updateHUD() {
  ...
  document.getElementById('prog').style.width = Math.round(resolvedSet.size/ALL_INCIDENTS.length*100)+'%';
}
function getTimeStr() {
  const h=Math.floor(gameTime/60), m=gameTime%60;
...
function addLog(time, msg, ok) {
  ...
  entry.innerHTML=`<span class="log-time">${time}</span><span class="${ok?'log-ok':'log-bad'}">${ok?'✓':'✗'} ${msg}</span>`;
...
function endGame() {
  ...
  if      (pct>=0.83&&health>=70) { grade='A+';icon='★';title='Outstanding Test Manager';    msg="You applied ISO 29119-2 flawlessly. The project was released safely."; }
  else if (pct>=0.67&&health>=50) { grade='A'; icon='✓';title='Successful Crisis Management';msg='You applied standard processes well — a few areas to improve.'; }
...
function renderLB() {
  ...
  if (lb.length===0) {
    document.getElementById('lb-list').innerHTML=`<div style="text-align:center;padding:20px;font-size:13px;color:var(--text-secondary)">No entries yet. Be the first to play!</div>`;
    return;
  }
  document.getElementById('lb-list').innerHTML=lb.slice(0,30).map((e,i)=>{
    ...
    return `<div class="lb-row ${medals[i]||''} ${isMe?'me':''}">
      ...
      <div><div class="lb-score">${e.score}</div><div class="lb-grade">${e.grade}</div></div>
    </div>`;
  }).join('');
}
